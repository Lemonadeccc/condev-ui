...
import { IconErrorLine, IconInfoLine, IconLoadingLine, IconSuccessLine, IconWarningLine } from '@condev-ui/ui';
...
export const CdAlert: React.FC<AlertProps> = (props) => {
  ...
  return (
    <div className={`pointer-events-auto ${containerClassName}`}>
      <div
        className={cn('relative flex items-center justify-between p-3 bg-white rounded-[10px] w-60 md:w-80 overflow-hidden', {
      ...
      >
        <div className="flex items-start gap-2 flex-1">
          ...
          >
            {type === 'info' && !icon && <IconInfoLine className={cn('text-xl text-stone-500', { 'mt-1': title })} />}
            {type === 'success' && !icon && <IconSuccessLine className={cn('text-xl text-green-600', { 'mt-1': title })} />}
            {type === 'warning' && !icon && <IconWarningLine className={cn('text-xl text-amber-500', { 'mt-1': title })} />}
            {type === 'error' && !icon && <IconErrorLine className={cn('text-xl text-red-600', { 'mt-1': title })} />}
            {type === 'loading' && !icon && <IconLoadingLine className={cn('text-gray-500 mt-1 animate-spin', { 'mt-1': title })} />}
          </motion.div>
        ...
        {typeof toastPercent === 'number' && toastPercent > 0 && (
          <div className="absolute bottom-[0] left-0 w-full h-1 bg-color-100">
            <div
              className={cn('h-full transition-[width] duration-300 ease-linear', {
